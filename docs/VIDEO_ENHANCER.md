...
MediaKeepa applies narrow build-time compatibility fixes to the pinned official 7B code. ByteDance's still-image branch omits an FPS bookkeeping entry and therefore skips its output loop, so the patch supplies `1.0` FPS for that one-frame path. The official transform also operates on 16-pixel boundaries; MediaKeepa renders at or above the requested size and center-crops the restored tensor before the official encoder writes it. At the official DiT/VAE device transitions, MediaKeepa moves the rank-zero color reference to CPU, releases offloaded CUDA allocator cache before the next model phase, and avoids reloading the DiT while decoded full-resolution frames are still live. These changes do not alter the checkpoint, model precision, VAE configuration, sampling, or restored pixels.
...
SeedVR2 restores video frames and writes the enhanced video stream. Longer or high-resolution videos are processed as overlapping, pixel-load-aware temporal windows through the same official model. Each join retains only the stable center frames, while surrounding frames provide temporal context. Every window runs in an isolated official distributed process group so CUDA and distributed IPC allocations are completely released before the next window. If the first memory estimate is still too large for the actual content and allocated GPU, MediaKeepa catches only CUDA out-of-memory failures and retries a fresh temporal plan with progressively smaller windows, down to SeedVR2's 17-frame minimum for four-GPU sequence parallelism. Window inputs and model outputs use lossless FFV1 intermediates; there is no spatial tiling, downscale, quantization, or smaller-model fallback. The frame-exact result is encoded once at visually lossless H.264 CRF 10.

MediaKeepa then remuxes the original audio and metadata into the final MP4. It first attempts a bit-for-bit audio stream copy. If the source codec cannot be represented safely in MP4, only the audio is converted to 320 kbps AAC; the restored video stream is never re-encoded during muxing.
...
The upstream project describes SeedVR2 as a prototype and notes that very heavy degradation, large motion, or lightly degraded low-resolution inputs can produce incomplete restoration or oversharpened detail. Official documentation currently describes four-H100 support through 1080p/2K. MediaKeepa exposes 4K as an experimental quality-first target on four Hopper GPUs. Its temporal window size automatically contracts for larger output frames, for 80 GB H100 workers, and after a measured CUDA memory failure, so memory safety does not require spatial tiling, quantization, or resolution reduction. If even the 17-frame full-quality window exhausts memory, the job reports that exact limit clearly.
